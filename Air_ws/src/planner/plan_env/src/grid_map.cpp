...
//double dx = pos(0) - md_.ugv_odom_buf_[i].pos.x()
    ...
    send_box_max = md_.ugv_odom_buf_[i].pos + mp_.send_update_range_;
    send_box_min = md_.ugv_odom_buf_[i].pos - mp_.send_update_range_;
    ...
    else
    {
      ...
      {
        send_box_max(0) = md_.ugv_odom_buf_[i].pos(0) + (7.0/4)*mp_.send_update_range_(0);
        send_box_min(0) = md_.ugv_odom_buf_[i].pos(0) - (1.0/4)*mp_.send_update_range_(0);
        sendmap.id_from = -1;
      }
      else if((dir<=M_PI && dir>=3*M_PI/4) || (dir>=-M_PI && dir<=-3*M_PI/4))
      {
        send_box_max(0) = md_.ugv_odom_buf_[i].pos(0) + (1.0/4)*mp_.send_update_range_(0);
        send_box_min(0) = md_.ugv_odom_buf_[i].pos(0) - (7.0/4)*mp_.send_update_range_(0);
        sendmap.id_from = -2;
      }
      else if(dir> M_PI/4 && dir< 3*M_PI/4)
      {
        send_box_max(1) = md_.ugv_odom_buf_[i].pos(1) + (7.0/4)*mp_.send_update_range_(1);
        send_box_min(1) = md_.ugv_odom_buf_[i].pos(1) - (1.0/4)*mp_.send_update_range_(1);
        sendmap.id_from = -3;
      }
      else if(dir< -M_PI/4 && dir> -3*M_PI/4)
      {
        send_box_max(1) = md_.ugv_odom_buf_[i].pos(1) + (1.0/4)*mp_.send_update_range_(1);
        send_box_min(1) = md_.ugv_odom_buf_[i].pos(1) - (7.0/4)*mp_.send_update_range_(1);
        sendmap.id_from = -4;
      }
    }
